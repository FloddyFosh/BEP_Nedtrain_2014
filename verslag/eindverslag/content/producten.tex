...
Na een analyse van het aantal voorrangsrelaties dat toegevoegd wordt en de flexibiliteit van elke heuristiek, is er uiteindelijk besloten om de heuristiek zoals beschreven in paragraaf \ref{subsubsec:chainingoplossing} als standaard te gebruiken voor de NedTrain Planner. Deze analyse en de precieze uitkomsten zullen verder worden toegelicht in paragraaf \ref{subsubsec:chaininganalyse}. Door een kleine aanpassing te maken in de source code kan er nog steeds gewisseld worden tussen de drie heuristieken.
...
Naast het resource profile dat weergegeven kan worden in de onderste helft van de interface \emph{(zie Figuur \ref{fig:GUIfinal})}, is er nu ook een nieuwe manier van het weergeven van de resources die worden gebruikt. Deze nieuwe feature maakt het mogelijk de chains, die worden gegenereerd door het chaining algoritme, op een eenvoudige manier te begrijpen en te visualiseren. De chains van resource units resulteren in een rooster, dat in de onderste helft van de interface kan worden weergegeven \emph{(zie Figuur \ref{fig:rooster})} met behulp van een nieuwe knop. Vooral als de resource units bijvoorbeeld monteurs, schoonmakers of ander personeel zijn is deze feature erg handig. Dit omdat de planning en de werknemers zelf kunnen zien wanneer wie aan welke activiteit werkt of pauze heeft. 
