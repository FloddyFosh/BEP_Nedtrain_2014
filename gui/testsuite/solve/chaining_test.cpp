...
TEST_F(ChainingTest, testGetSetName) {
...
TEST_F(ChainingTest, testGetBinary) {
...
TEST_F(ChainingTest, testGetArguments) {
...
TEST_F(ChainingTest, testSaveAndLoad) {
    s->save();
...
TEST_F(ChainingTest, testFailingLoad) {
    ASSERT_THROW(Solver::load("UnexistingSolver"), SolverNotFoundException);
}

TEST_F(ChainingTest, testFinished) {
...
TEST_F(ChainingTest, testSecondSolveWithoutClearing) {
    QProcess* solveProcess = s->getProcess();
...
TEST_F(ChainingTest, testSecondSolveWithClearing) {
    QProcess* solveProcess = s->getProcess();
    QProcess* solveProcess2 = s2->getProcess();
