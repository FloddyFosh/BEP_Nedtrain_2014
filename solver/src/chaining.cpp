...
void add_frame() {
    ...
    FOREACH(activities, it){
        ...
        i = act->i, j = act->j;
        activity* a = A(i,j);
        fprintf(stdout, " %d %d %d %d", i, a->est, a->lst + a->flex, (len(a->groupchilds)+1));
        fprintf(stdout, " %d %d", act->i, act->j);
...
void print_chain(int i, int j) {
    ...
    fprintf(stdout, " %d %d %d",i,j,len(chain));
    FOREACH(chain, it){
        activity* act = *it;
        //<job/train> <activity>
        fprintf(stdout, " %d %d",act->i,act->j);
